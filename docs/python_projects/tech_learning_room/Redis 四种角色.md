...
对照 creativault 线上实现的教学复刻，零业务耦合：把 creati-vault 里 Redis 同时承担的四种角色抽成独立、可运行的面向对象代码。
...
「一个 Redis 多角色」的本质：**物理上同一个实例，靠 key 前缀 + 数据结构 + 访问模式划分出互不干扰的逻辑通道**。
...
cp .env.example .env          # 按需改 REDIS_URL（compose 提供的实例在 localhost:6380）
...
Celery 的 countdown 是「消息层面的延迟」（worker 收到后按 ETA 持有），长延迟任务会占住 unacked 消息、受 visibility_timeout 反复重投。自建延迟队列是「存储层面的延迟」：
...
相关进阶教程：缓存三大问题与失效竞态见 [redis-cache-consistency](Redis 缓存一致性.md)；Celery 可靠性参数矩阵见 [celery-task-reliability](Celery 任务可靠性.md)。
