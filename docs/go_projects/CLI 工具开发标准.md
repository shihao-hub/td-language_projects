...
**新工具默认交付 CLI 与 MCP 两个入口，共用业务核心；CLI 默认面向人，`--json` 面向自动化。**
...
| 项目 | 统一要求 |
|---|---|
| 设计顺序 | 业务用例与行为契约 → 渠道映射 → 公共核心与适配器；逐用例迭代 |
| 双入口 | 默认配套 MCP；本地工具默认通过同一程序的 `mcp` 子命令提供 stdio server |
| CLI 输出 | 管理、查询类命令默认人读，必须提供稳定的 `--json` 模式 |
| 离线发现 | 必须提供 `<prog> schema`，导出与 MCP 注册同源的契约 |
| 自描述 | 必须提供 `--help`、子命令帮助和 `--version`；说明参数、默认值、输出及副作用 |
| 公共核心 | 归一化、业务校验、状态变化、执行编排、记账和业务错误有共同归属 |
| 入口职责 | CLI/MCP 负责解析、渠道策略、协议适配与渲染；不能各写一套业务流程 |
| 兼容 | 存量 CLI 的公开行为按原契约保留，按用例逐步迁移 |
| 验收 | 用例行为、CLI 子进程和真实 MCP stdio 均有对应证据；仅能连接不算完成 |
...
- 项目文档放父仓库 `docs/<lang>/<项目相对路径>/`；本标准统一保留在 `docs/go_projects/`，其他语言引用同一文件，不复制一份。
...
flowchart TB
    ...
    H["MCP 客户端"] -->|"协议消息"| M["MCP 适配器"]
    C -->|"业务请求与执行上下文"| S["公共用例：校验、编排、状态与错误"]
    M -->|"业务请求与执行上下文"| S
    G["按需增加 GUI / HTTP 入口"] -->|"业务请求"| S
    S -->|"通过接口读写与执行"| I["存储 / 进程执行器 / 远程 API"]
    I -->|"数据、事件与执行结果"| S
    S -->|"结果或业务错误"| C
    S -->|"结果或业务错误"| M
```
...
默认工具名采用 `<prog>.<verb>`，在同一 server 内唯一；聚合端用 server 身份与工具名共同定位。说明至少包括用途、参数单位与缺省、重要限制和副作用。
