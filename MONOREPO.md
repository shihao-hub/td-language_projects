...
|-|-|-|-|-|-|
| cli-json-reel | 用纯 Canvas2D 确定性离线渲染的排版动画短片，主题为 Go CLI 的 JSON 输出模式 |  |  |  |  |
| django-lab-react | 图书馆业务系统的 React SPA 前端，消费 django-lab 后端 /api/ 接口，前后端分离形态 |  |  |  |  |
| frontend-learning-guide | 后端开发者转向前端全栈（TS/React/Next.js）的学习仓库，平铺 monorepo 管理 |  |  |  |  |
| jtree | 通用 JSON → 终端树形渲染器，stdout 为终端时自动进入交互式浏览 TUI |  |  |  |  |
| playwright-demo | Playwright E2E 测试 demo：对父仓库 docs 静态站点做端到端验收 |  |  |  |  |
| taskmon | 更好用的任务管理器 | 稳定，长期使用 | 未阅读，核心链路未知 |  |  |
| text-extractor | chrome 插件开发初体验 | 可用 | 已阅读，较简单，但是又属于跨领域知识了，好处是公司也有达人插件开发，可以用来优化简历 |  |  |
| tooldeck | Inspector 式 MCP 工具面板（Electron + React），协议自描述驱动工具发现与执行 |  |  |  |  |
| tooldeck-tauri | Tauri 2 复刻 tooldeck 的占位目录，当前无代码，待 Electron 版稳定后评估 |  |  |  |  |
...
> 子模块未初始化：`.gitmodules` 已声明，但父仓索引无 gitlink、本地无目录；下表条目为规划记录。

| 项目 | 摘要 | 状态 | 源码阅读情况 | 价值 | 下一步 |
|-|-|-|-|-|-|
| trilang | C/C++/Lua 三语言协作 demo：C 算法库（fib/gcd）+ C++ 宿主嵌入 Lua 5.1.5 双向调用 | 可用 |  |  |  |
...
| agyquota | Antigravity 模型配额查询 CLI，不打开 IDE 即可查各模型桶剩余百分比与重置时间 | 归档 |  |  |  |
...
| exestarter | exe 收藏架 CLI：扫描收集、集中注册、透传启动/定位/开终端（exe-launcher 的 CLI 版） | 归档 |  |  |  |
